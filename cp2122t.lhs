...
\\
a95088 & José Miguel Ferreira Barbosa
\\
...
%
     	...
     	|lcbr(
     		...
     		k2 = succ.(π2.π1), c != const 0
     	)|
     	\\
     	|lcbr(
     	  	l2 = const d, c = const 0
     	)(
     		l2 = pred.(π2,π2), c != const 0
     	)|
     	\\
\qed
\end{eqnarray*}
...
Através do Forkinga:  split(alice, bob) = cata(split(h,k))
...
Em que: 
g1 = dup \\
g2 = split([id,uncurry(max) . (π2 x π2)], ([id,uncurry(min).(π1 x π1)])
...
\\
 ...
 trig1: Triangulo formado quando somamos side/2 à coordenada x original e o new side = side/2 \\
 trig2: Triangulo formado quando somamo side/2 à coordenada y original e o new side = side/2 \\
...
\end{code}
\\
1º passo: aplicar a função bflip á cabeça da lista de bits e obter uma distribuição da cabeça\\
\\
2º passo: aplicar a função propagate á cauda da lista e obter uma distribuição da lista\\
\\
3º passo: juntar a distribuição da cabeça com a distribuição da lista obtendo uma distribuição final\\
\\
4º repetir o processo para todos os elemnetos da lista\\ 
\begin{code}
...
\end{code}
\\
1º passo: aplicar a função bflip3 á cabeça da lista (esta será repetida 3 vezes)\\
\\
2º passo: aplicar um fmap com a função v3 á Dist Bit3 da cabeça convertendo-a para uma Dist Bit
\\
\\
3º passo: aplicar a função propagate3 á cauda da lista e obter uma Dist [Bit]\\
\\
4º passo: juntar a distribuição da cabeça com a distribuição da lista obtendo uma distribuição final\\
\\
5º repetir o processo para todos os elementos da lista 
\\
